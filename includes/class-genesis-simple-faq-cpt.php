<?php
/**
 * Genesis Simple FAQ Custom Post Type class.
 *
...
class Genesis_Simple_FAQ_CPT {
	...
	public function cpt_updated_messages( $messages ) {
		...
		/* translators: Revision version. */
		$revision = isset( $_GET['revision'] ) ? sprintf( __( 'FAQ restored to revision from %s', 'genesis-simple-faq' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		...
		$messages['gs_faq'] = array(
			...
			9  => sprintf(
				/* translators: Date publish to publish the FAQ */
	 ...
	 * @since 0.9.0
	 */
	public function faq_shortcode_column_content( $column_name, $post_ID ) {

		if ( 'gs_faq' === $column_name ) {
			echo '[gs_faq id="' . esc_attr( $post_ID ) . '"]';
		}
